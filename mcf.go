...
func Create(plaintext []byte) (encoded []byte, err error) {
	if !defaultEncoding.IsValid() {
...
func Verify(plaintext, encoded []byte) (isValid bool, err error) {
	_, enc := findInstance(encoded)
	if enc == nil {
		return false, &ErrNoEncoder{string(encoded)}
	}
...
func IsCurrent(encoded []byte) (isCurrent bool, err error) {
	encoding, enc := findInstance(encoded)
	if enc == nil {
		err = &ErrNoEncoder{string(encoded)}
	} else {
